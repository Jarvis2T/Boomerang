<?php  
	...
	$sql="SELECT * FROM product_detail,product_type WHERE product_type.id_pdtype=product_detail.id_pdtype ORDER BY product_detail.id_pddetail DESC LIMIT $page,5";
	$run=mysqli_query($conn,$sql);

	//Pagination
	$sql_pagination="SELECT * FROM product_detail,product_type WHERE product_type.id_pdtype=product_detail.id_pdtype";
	...
	if(isset($_POST['search_btn'])){
		...
		header('location:index.php?management=product_detail&p=search&search='.$search.'&page=1');
	}
?>
...
<div class="page row">
	<nav aria-label="Page navigation" class="pages_divider">
	  	<ul class="pagination">
	    	<li>
		      	<a <?php if($get_page=='' || $get_page==1){ ?> class="pre btn disabled" <?php } ?> href="?management=product_detail&page=<?php echo $get_page-1 ?>" aria-label="Previous">
	    	...
	    	<?php  
	    		for($i=1;$i<=$each;$i++){
	    	?>
	    			<li><a <?php if($i==$get_page){ ?> class="active" <?php } ?> href="?management=product_detail&page=<?php echo $i ?>"><?php echo $i ?></a></li>
	    	<?php  
	    		}
	    	?>
	    	<li>
		      	<a <?php if($get_page=='' || $get_page==$each){ ?> class="next btn disabled" <?php } ?> href="?management=product_detail&page=<?php echo $get_page+1 ?>" aria-label="Next">
